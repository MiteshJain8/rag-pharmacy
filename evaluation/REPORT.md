...
For deployment, Groq and Cohere keys are intentionally absent from `render.yaml`, so the public demo uses deterministic extracts with no provider calls. Performance on Render's free instance, including cold starts, was not included in the 60-question benchmark.

The Docker image built locally on 2026-09-25. A container run with both provider keys disabled returned HTTP 200 from `/health`, `/ready`, `/`, and a real CDSCO API query. Warm container memory read 174.3 MiB after that query on the local Docker host. This is a local smoke test, not Render capacity or public URL verification.

## Public deployment verification

Render deployed commit `fccac20` on the free 512 MB instance in Singapore at [https://rag-pharmacy.onrender.com/](https://rag-pharmacy.onrender.com/) on 2026-09-25. The live browser UI returned five source cards for `CDSCO banned Phenacetin`, led by `cdsco-banned-8`. Direct public checks returned HTTP 200 from `/health` (`environment: production`), `/ready`, and `/api/v1/query`. The safe query returned five sources with no confidence field. `Can I replace my medicine with paracetamol?` returned HTTP 200 with zero sources and a refusal. Render's saved environment variable names contained the Supabase publishable key and no service-role key. These are smoke checks, not a public load test or clinical review.
